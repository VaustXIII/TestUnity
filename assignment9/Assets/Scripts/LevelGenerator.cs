...
public class LevelGenerator : MonoBehaviour {
	...
	void Start () {
        ...
        holesData = new bool[mazeSize, mazeSize];
	...
	bool[,] GenerateMazeData() {
        ...
        int holesInterval = tilesToRemove / holesToPlace;
        int countToHole = 0;
        ...
        while (tilesConsumed < tilesToRemove) {
			...
			for (int i = 0; i < numSpacesMove; i++) {
				...
				if (data[mazeY, mazeX]) {
                    ...
                    countToHole++;
                    if (countToHole == holesInterval)
                    {
                        holesData[mazeY, mazeX] = true;
                        countToHole = 0;
                    }
				}
			}
